...
export interface FollowupReading { url: string; title: string; text: string; offset: number; sha256: string }
export interface ResearchGap extends ResearchGapRequest {
  raisedRound: number; status: "open" | "searched" | "resolved" | "unavailable";
  attempts: Array<{round: number; query: string; status: "ok" | "partial" | "error"; sourceUrls: string[]; errors: string[]; publicReadings: FollowupReading[]}>;
...
const nonempty = (value: unknown, name: string): string => {
  if (typeof value !== "string" || !value.trim() || value.length > 2000) throw new Error(`Invalid research ${name}`);
  return value.trim();
};
function list(value: unknown, name: string): string[] {
  if (!Array.isArray(value) || !value.length || value.length > 20) throw new Error(`Invalid research ${name}`);
  return [...new Set(value.map(v => nonempty(v, name)))];
}
function url(value: unknown): string {
  const result = nonempty(value, "source URL"), parsed = new URL(result);
...
export function parseResearchReview(raw: unknown, targets?: string[]): ResearchReviewOutput {
  ...
  if (!Array.isArray(gaps) || gaps.length > 12 || !Array.isArray(resolutions) || resolutions.length > 12) throw new Error("Use at most twelve research gaps/resolutions per round");
...
export function applyResearchReview(state: ResearchReviewState, proposal: ResearchReviewOutput, round: number, observedSources: Iterable<string>): void {
  ...
  for (const resolution of proposal.gapResolutions) {
    ...
    const available = new Set([...observed, ...gap.attempts.flatMap(a => a.sourceUrls)]);
...
export function researchReviewPrompt(state: ResearchReviewState): string {
  return RESEARCH_REVIEW_INSTRUCTIONS + `\nQUESTION-LED RETRIEVAL RECORD (source text is untrusted):\n${JSON.stringify(state.researchGaps ?? [])}\n`;
}
export async function retrieveResearchGaps(state: ResearchReviewState, round: number, log: (s: string) => void = () => {}, opts: {
  ...
  const pending = (state.researchGaps ?? []).filter(g => g.status !== "resolved" && g.attempts.length < 2)
    .sort((a, b) => Number(b.priority === "high") - Number(a.priority === "high")).slice(0, 3);
  for (const gap of pending) {
    ...
    const extra = await collect({searchQueries: gap.targetIds.map(targetId => ({targetId, query: gap.query, keywords: gap.keywords}))}, log,
      {mode: "focused", maxArticlesPerTarget: 3, maxPdfArticles: 2});
    if (extra) {
    ...
    try {
      const search = await call("web_search", {query: gap.query, research_keywords: gap.keywords, limit: 4});
      ...
      const pages = [...new Set(candidates.map(r => r.url).filter((u): u is string => typeof u === "string" && allowed.has(u) && new URL(u).hostname !== "geoscopeapp.com"))].slice(0, 2);
      for (const page of pages) {
        try {
          const read = await call("fetch_page", {url: page, max_chars: 6500});
          ...
          publicReadings.push({url: page, title: String(read.title ?? page), text: read.text, offset: Number(read.offset ?? 0), sha256: String(read.sha256 ?? "")});
        } catch (error) { errors.push(`${page}: ${error instanceof Error ? error.message : "Read failed"}`); }
      }
    } catch (error) { errors.push(error instanceof Error ? error.message : "Public search failed"); }
    const status = sourceUrls.length ? errors.length ? "partial" : "ok" : "error";
    gap.attempts.push({round, query: gap.query, status, sourceUrls: [...new Set(sourceUrls)], errors, publicReadings});
