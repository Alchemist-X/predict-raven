...
> 当前状态：研究支持二元概率、互斥选项、数值与逐实体事件排名，并保留原答案口径。CLI/API/MCP/Raven 默认不限研究轮次；检索失败、证据不足、预算耗尽均明确展示未完成及暂定估计。扩展资源库采用目录发现、实际正文阅读与重大疑问回搜，正文重要图片按当前问题选择并保存视觉观察；纯文字 DeepSeek 路线保留未读图缺口。公开仓库只保留通用适配器，订阅缓存与凭证留在个人工作区。本批合并验收：386 项引擎/API/Raven 测试、三处类型检查、Raven production build 通过；桌面/手机 20 个页面场景无浏览器错误或横向溢出，并人工读图复核。未运行新的付费模型研究。详见[流程与接入](signal-desk-research.md)及[合并验收](internal/review/2026-09-15-research-release.md)。
