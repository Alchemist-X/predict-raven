...
> 当前新增工作：原生分类、数值与独立事件排名已接入引擎/API/MCP/Raven，个人研究强制使用扩展资源库；详见[答案形式](forecast-answer-types.md)。新状态是 schemaVersion 2，不能交给二元交易消费者。引擎/API 的 277 项测试、三处类型检查及 API/MCP/网页多类型验收通过；M7 实际验收及私人证据留个人工作区。
